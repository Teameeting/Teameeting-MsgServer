...
#		   
LOCAL_LDLIBS := -llog
####LOCAL_LDLIBS += -L$(call host-path,$(LOCAL_PATH)/lib_static)/ librtcmsgclient-android.a
####LOCAL_LDFLAGS += lib_static/librtcmsgclient-android.a
LOCAL_CFLAGS := -DWEBRTC_POSIX -D__STDC_CONSTANT_MACROS -std=gnu++11 -fno-rtti

LOCAL_C_INCLUDES += $(NDK_STL_INC) \
					$(LOCAL_PATH)/ \
					$(LOCAL_PATH)/jutils \
					$(LOCAL_PATH)/RTCMsgClient/ \
					$(LOCAL_PATH)/RTCMsgClient/rtcmsgs \

										
#LOCAL_SHARED_LIBRARIES := jni_utils

LOCAL_STATIC_LIBRARIES := jni_utils
LOCAL_STATIC_LIBRARIES += msgclient-tmp
