...
class StrPtrLen
{
    public:
        ...
        void Delete() { if(Ptr)delete [] Ptr; Ptr = NULL; Len = 0; }
        ...
        virtual void Set(char* inPtr, UInt32 inLen) { Ptr = inPtr; Len = inLen; }
        virtual void Set(char* inPtr) { Ptr = inPtr; Len = (inPtr) ?  ::strlen(inPtr) : 0; }
...
class StrPtrLenDel : public StrPtrLen
{
  public:
	 ...
	 StrPtrLenDel(const char* sp) : StrPtrLen(NULL) {Set(sp);}
     StrPtrLenDel(const char *sp, UInt32 len) : StrPtrLen(NULL,0) {Set(sp, len);}
	 StrPtrLenDel(StrPtrLen* strPtr): StrPtrLen(NULL,0){if(NULL !=strPtr)Set(strPtr->Ptr, strPtr->Len);}
     ~StrPtrLenDel() { Delete(); }

	StrPtrLenDel& operator=(const StrPtrLen& newStr) { Set(newStr.Ptr, newStr.Len); return *this; }
	virtual void Set(const char* inPtr, UInt32 inLen);
    virtual void Set(const char* inPtr);
};
