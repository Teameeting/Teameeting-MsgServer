...
Task* RRTRTRedisListener::GetSessionTask(int osSocket, struct sockaddr_in* addr)
{
    ...
    RRTTransferSession* theTask = new RRTTransferSession();
    if(NULL == theTask)
        return NULL;
    ...
    LI("RRTRTRedisListener Get a connection,ip:%.*s port:%d \n",remoteStr->Len, remoteStr->Ptr, ntohs(addr->sin_port));
