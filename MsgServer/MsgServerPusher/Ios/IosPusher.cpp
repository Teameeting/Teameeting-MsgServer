...
bool IosPusher::InitPusher(const char* p12_dir, const char* p12_file, const char* pass)
{
    if (!p12_dir || !p12_file || !pass) return false;
    ...
    char p12_path[256] = {0};
    sprintf(p12_path, "%s/%s", p12_dir, p12_file);
    ...
    apn_set_pkcs12_file(mApnCtx, p12_path, pass);
