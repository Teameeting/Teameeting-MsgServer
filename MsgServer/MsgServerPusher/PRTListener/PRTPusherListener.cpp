...
Task* PRTPusherListener::GetSessionTask(int osSocket, struct sockaddr_in* addr)
{
    ...
    PRTTransferSession* theTask = new PRTTransferSession();
    if(NULL == theTask)
        return NULL;
    ...
    //LI("PRTPusherListener Get a connection,ip:%.*s port:%d \n",remoteStr->Len, remoteStr->Ptr, ntohs(addr->sin_port));
