...
static RTConnector*	g_pConnector = NULL;

void RTConnector::PrintVersion()
{
...
void RTConnector::Initialize(int evTbSize)
{
	...
	OSThread::Sleep(100);
	g_pConnector = new RTConnector();
	g_inited = true;
}

void RTConnector::DeInitialize()
{
	delete g_pConnector;
	g_pConnector = NULL;
	g_inited = false;
...
RTConnector* RTConnector::Inst()
{
	Assert(g_pConnector != NULL);
	return g_pConnector;
}

RTConnector::RTConnector(void)
