...
Task* GRTGrouperListener::GetSessionTask(int osSocket, struct sockaddr_in* addr)
{
    ...
    GRTTransferSession* theTask = new GRTTransferSession();
    if(NULL == theTask)
        return NULL;
    ...
    //LI("GRTGrouperListener Get a connection,ip:%.*s port:%d \n",remoteStr->Len, remoteStr->Ptr, ntohs(addr->sin_port));
