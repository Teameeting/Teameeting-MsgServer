...
void CORTConnHttp::OnParsedHttpMessage(const void*pUserData, http_message* httpMsg)
{
	CORTConnHttp* pHttp = (CORTConnHttp*)pUserData;
	if(pHttp)
	{
...
CORTConnHttp::CORTConnHttp(void)
: m_bAuthed(false)
, m_nTimer(10)
...
int CORTConnHttp::ProcessData(const char*pData, int nLen)
{
	int parsed = 0;
	enum http_errno err;
	while (parsed < nLen)
	{
		...
		int nlen = do_http_parse(HTTP_RESPONSE, (char *)pMsg, msgLen, &err, &CORTConnHttp::OnParsedHttpMessage, this);
		parsed += nlen;
		if (err != HPE_OK)
		{
...
void CORTConnHttp::OnHttpMessage(http_message* httpMsg)
{
...
const char* CORTConnHttp::GenerateRequest(http_method method, const std::string& path, const std::string& data, int &outLen)
{
    if (path.length()==0) {
        LE("params error!!!\n");
        outLen = 0;
...
const char* CORTConnHttp::GenerateRequest(http_method method, const std::string& path, const std::string& data, int &outLen) const
{
    if (path.length()==0) {
        LE("params error!!!\n");
        outLen = 0;
