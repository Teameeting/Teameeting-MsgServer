...
Task* CRTModuleListener::GetSessionTask(int osSocket, struct sockaddr_in* addr)
{
    ...
    CRTTransferSession* theTask = new CRTTransferSession();
    if(NULL == theTask)
        return NULL;
    ...
    LI("CRTModuleListener Get a connection,ip:%.*s port:%d \n",remoteStr->Len, remoteStr->Ptr, ntohs(addr->sin_port));
