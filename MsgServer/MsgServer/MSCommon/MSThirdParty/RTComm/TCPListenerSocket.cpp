...
void TCPListenerSocket::ProcessEvent(int /*eventBits*/)
{
	...
	//while(1)
	{
		osSocket = accept(fFileDesc, (struct sockaddr*)&addr, &size);
        
        qtss_printf("TCPListenerSocket::ProcessEvent osSocket:%d, fFileDesc:%d\n", osSocket, fFileDesc);
	...
	//test acceptError = ENOENT;
			...
			{   
				...
				theTask = this->GetSessionTask(osSocket, &addr);
				if (theTask == NULL)
				{   
					close(osSocket);
				}
