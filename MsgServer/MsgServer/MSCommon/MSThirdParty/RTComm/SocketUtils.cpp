...
void SocketUtils::UnInitialize(void)
{
	if (sIPAddrInfoArray)
	{
...
UInt32 SocketUtils::GetLocalIPAddr()
{
	char        buf[256];
    struct hostent  *hent;
    int         tryCount = 0;
...
again:
    ...
    localIP = ntohl(((struct in_addr *)hent->h_addr)->s_addr);
    return localIP;
}
...
UInt32 SocketUtils::ConvertDnsToAddr(const char* inDnsName)
{
	if (inDnsName == NULL)
        return 0;
