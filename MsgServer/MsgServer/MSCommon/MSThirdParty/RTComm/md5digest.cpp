...
void ComputeMD5(const char *pw, int len, char *result)
{
	unsigned char m_Buf[32];
	MD5_CTX m_Context;
	MD5_Init(&m_Context);
	MD5_Update(&m_Context,(unsigned char *)pw,len);
	MD5_Final(m_Buf, &m_Context);
	char* p = (char *)m_Buf;
	for (int i=0; i < 16; i++)
	{
		...
		int hi = (temp & 0xf0)>>4;
		int low = (temp & 0xf);
		result[i*2] = hexmap[hi];
		result[i*2+1] = hexmap[low];
	}
} 
