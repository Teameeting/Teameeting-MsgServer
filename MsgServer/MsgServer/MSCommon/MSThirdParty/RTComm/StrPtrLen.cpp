...
void StrPtrLenDel::Set(const char* inPtr, UInt32 inLen)
{
	if(inPtr == NULL || inLen <= 0)
		return;

	Delete();

	Ptr = new char[inLen + 1];
	Len = inLen;
	memcpy(Ptr, inPtr, inLen);
	Ptr[inLen] = '\0';
}

void StrPtrLenDel::Set(const char* inPtr)
{
    UInt32 inLen = inPtr==NULL?0:strlen(inPtr);
    Set(inPtr, inLen);
}
