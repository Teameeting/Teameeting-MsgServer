...
Task* MRTModuleListener::GetSessionTask(int osSocket, struct sockaddr_in* addr)
{
    ...
    MRTTransferSession* theTask = new MRTTransferSession();
    if(NULL == theTask)
        return NULL;
    ...
    //LI("MRTModuleListener Get a connection,ip:%.*s port:%d \n",remoteStr->Len, remoteStr->Ptr, ntohs(addr->sin_port));
