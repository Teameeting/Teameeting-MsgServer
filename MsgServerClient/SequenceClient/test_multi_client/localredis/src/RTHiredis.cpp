...
bool RTHiredis::CmdGet(const std::string key, long long* value)
{
    ...
    } else if (m_redisReply->type==REDIS_REPLY_STRING)
    {
        ...
        *value = strtoll(m_redisReply->str, &endptr, 10);
        printf("CmdGet longlong string str:%s\n", m_redisReply->str);
    } else
    {
        printf("CmdGet longlong  m_redisReply->str is null, key is:%s\n", key.c_str());
    }
...
bool RTHiredis::GenericIncrId(const std::string& key, long long* value)
{
    ...
    if (!m_redisContext || key.empty()) {
        if (!m_redisContext) printf("GenericIncrId m_redisContext is null, this:%p\n", this);
        if (key.empty()) printf("GenericIncrId key is empty, this:%p\n");
    ...
    {
        printf("GenericIncrId m_redisReply->str is null, key is:%s\n", key.c_str());
    }
    freeReplyObject((void*)m_redisReply);
    return true;
}
